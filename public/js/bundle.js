...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    // Create points
    points.push([loc.coordinates[1], loc.coordinates[0]]); // Create Popup

    var popup = L.popup().setLatLng([loc.coordinates[1], loc.coordinates[0]]).setContent("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).openOn(map); // Add marker(LatLng)
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            stripe = Stripe('pk_test_51LIcSeEEui0IhzWGM2jgKm5zUckpPcsznMmwSNlZUkIwZdWmNOpjDel7IO7qWRpHvGqTRrq1qL9A0HxuNWO4xajF001Iz64qKr'); // 1) Get checkout session from API
